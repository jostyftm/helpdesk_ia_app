...
const mockChats = [
    { id: "1", user: "Alice Smith", text: "Hi, I tried bypassing the firewall as you...", time: "10:30 AM", unread: 2, status: "active", ticketId: "T-1001" },
    { id: "2", user: "Bob Taylor", text: "Thanks for the help, everything works now.", time: "Yesterday", unread: 0, status: "closed", ticketId: "T-1002" },
    { id: "3", user: "Charlie Davis", text: "When will the server be back online?", time: "2 Days ago", unread: 0, status: "closed", ticketId: "T-1003" },
];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-8 h-full flex flex-col">
            <div className="mb-6 shrink-0 bg-white/60 backdrop-blur-md p-6 rounded-2xl border border-white/40 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">Engage with customers in real-time to resolve issues faster.</p>
                </div>
                <div className="flex bg-slate-100 p-1 rounded-xl">
                    <button onClick={() => setFilter("all")} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${filter === "all" ? "bg-white shadow-sm text-indigo-700" : "text-slate-500 hover:text-slate-700"}`}>All</button>
                    <button onClick={() => setFilter("active")} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${filter === "active" ? "bg-white shadow-sm text-indigo-700" : "text-slate-500 hover:text-slate-700"}`}>Active</button>
                    <button onClick={() => setFilter("closed")} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${filter === "closed" ? "bg-white shadow-sm text-indigo-700" : "text-slate-500 hover:text-slate-700"}`}>Closed</button>
                </div>
            </div>
                ...
                <div className={`w-full md:w-1/3 border-r border-slate-200/60 flex flex-col bg-slate-50/30 ${activeChat ? 'hidden md:flex' : 'flex'}`}>
                    <div className="p-4 border-b border-slate-200/60">
                        <div className="relative">
                            ...
                            <input type="text" className="block w-full pl-10 sm:text-sm border-slate-200 rounded-xl py-2.5 px-3 bg-white focus:ring-indigo-500 focus:border-indigo-500 border shadow-sm transition-colors" placeholder="Search chats or tickets..." />
                        </div>
                    </div>
                        ...
                        <div className="flex items-end pr-4 sm:pr-16 animate-in slide-in-from-left-4 fade-in duration-300">
                            ...
                            <div className="max-w-[85%] sm:max-w-[75%]">
                                <div className="bg-white border border-slate-200/60 text-slate-700 text-sm p-3 sm:p-4 rounded-2xl rounded-bl-sm shadow-sm inline-block leading-relaxed break-words">
                                    Hi, I am having issues connecting to the VPN again. It says connection timed out.
                                </div>
                        ...
                        <div className="flex items-end pl-4 sm:pl-16 justify-end animate-in slide-in-from-right-4 fade-in duration-300">
                            <div className="flex flex-col items-end max-w-[85%] sm:max-w-[75%] w-full">
                                <div className="bg-indigo-600 text-white text-sm p-3 sm:p-4 rounded-2xl rounded-br-sm shadow-sm leading-relaxed self-end w-auto text-left break-words">
                                    Hello {selectedChatData?.user.split(" ")[0]}, I see the ticket you opened. Let me check the connection logs for your device.
                                </div>
                    ...
                    <div className="p-4 bg-white border-t border-slate-200/60 shrink-0">
                        {selectedChatData?.status === "closed" ? (
                            <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-200/60">
                                <p className="text-sm text-slate-500 font-medium">This conversation is closed. Reopen the ticket to send messages.</p>
                            </div>
                        ) : (
