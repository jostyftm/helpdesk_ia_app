...
import { ArrowLeft, Clock, MessageSquare, Paperclip, User, Bot, AlertCircle, CheckCircle, Search, Send, FileCheck, History, BarChart2, Edit, MoreHorizontal, Mail, Download, Edit3, Tag, MessageCircle, Info, Bold, Italic, Underline, List, ListOrdered, Link as LinkIcon, Image as ImageIcon, Code, AlignLeft, AlignCenter, AlignRight, Plus, Sparkles, ChevronRight, X, BarChart, Loader2 } from "lucide-react";
...
const ReactQuill = dynamic(() => import("react-quill-new"), { ssr: false, loading: () => <div className="h-64 bg-slate-50 animate-pulse rounded-md border border-slate-200"></div> });
...
export default function TicketDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { ticket, isLoading, error, getRequester, getTechnicians, refetch } = useTicketDetails(id);
    const { form: resolveForm, onSubmit: onResolveSubmit, isSubmitting: isResolving, globalError: resolveError } = useResolveTicket(id, refetch);
        ...
        return (
            <span
                ...
                style={{
                    ...
                    color: color || '#475569',
                    borderColor: bgColor ? `${bgColor}40` : '#e2e8f0'
                }}
            >
                {text}
            </span>
    ...
    if (error || !ticket) {
        return (
            <div className="flex flex-col h-full w-full items-center justify-center">
                ...
                <p className="text-slate-500">{error || 'Ticket no encontrado'}</p>
                <Link href="/dashboard/tickets" className="mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg">Volver</Link>
            </div>
        );
    }

    const { attributes, relationships } = ticket;
    const s = relationships?.current_state?.attributes;
    const p = relationships?.priority?.attributes;
    const c = relationships?.category?.attributes;
    const src = relationships?.source?.attributes;
    ...
    return (
        <div className="w-full px-4 sm:px-6 md:px-8 py-8 h-full flex flex-col space-y-6">
                ...
                <div className="flex items-start">
                    ...
                    <div>
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                            <span className="font-semibold text-slate-800 text-sm">TK-{ticket.id}</span>
                            {p ? (
                        ...
                        <div className="flex flex-col lg:flex-row gap-6">
                            ...
                            <div className="lg:w-[70%] space-y-6">
                                <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm">
                                    ...
                                    <div className="pt-4 flex items-center gap-6 text-sm text-slate-600 flex-wrap">
                                        <div className="flex items-center gap-1.5">
                                            <Tag className="h-4 w-4 text-slate-400" />
                                            Categoria: <span className="font-semibold text-slate-800">{c ? c.name : 'N/A'}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <User className="h-4 w-4 text-slate-400" />
                                            Reportado por: <span className="font-semibold text-slate-800">{requester ? `${requester.relationships.user.attributes.name} ${requester.relationships.user.attributes.last_name || ''}`.trim() : 'Desconocido'}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                ...
                                <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm">
                                    ...
                                    <div className="space-y-4 text-sm bg-white">
                                        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                                            ...
                                            {s ? <ColorBadge text={s.display_name} color={s.text_color} bgColor={s.bg_color} /> : <span className="text-slate-800">N/A</span>}
                                        </div>
                                        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                                            <span className="text-slate-500">Prioridad</span>
                                            {p ? <ColorBadge text={p.name} color={p.text_color} bgColor={p.bg_color} /> : <span className="text-slate-800">N/A</span>}
                                        </div>
                                        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                                            <span className="text-slate-500">Categoria</span>
                                            <span className="font-semibold text-slate-800">{c ? c.name : 'N/A'}</span>
                                        </div>
                                        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                                            <span className="text-slate-500">Origen</span>
                                            <span className="font-semibold text-slate-800">{src ? src.name : 'N/A'}</span>
                                        </div>
                                        ...
                                        <button 
                                            ...
                                            className="w-full mt-6 py-2.5 bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 text-slate-700 hover:text-indigo-700 font-semibold rounded-xl text-sm transition-all flex items-center justify-center gap-2"
                                        >
                                    ...
                                    <div className="space-y-4">
                                        {technicians.length > 0 ? (
                                            technicians.map(tech => (
                                                <div key={tech.relationships.user.id} className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-sm shrink-0 uppercase">
                                                        {tech.relationships.user.attributes.name[0]}{tech.relationships.user.attributes.last_name?.[0] || ''}
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-semibold text-slate-800">
                                                            {tech.relationships.user.attributes.name} {tech.relationships.user.attributes.last_name || ''}
                                                        </p>
                                                        <p className="text-xs text-slate-500 capitalize">{tech.relationships.role.attributes.name.replace(/_/g, ' ')}</p>
                                                    </div>
                                                </div>
                                            ))
                                        ) : (
                                            <p className="text-sm text-slate-500">No hay tecnicos asignados aun.</p>
                                        )}
                                        <button 
                                            onClick={() => setIsAssignTechnicianModalOpen(true)}
                                            className="w-full mt-2 py-2.5 text-sm font-medium text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors flex items-center justify-center gap-2"
                                        >
                                            <User className="h-4 w-4 text-slate-400" />
                                            Asignar tecnico
                                        </button>
                    ...
                    {activeTab === "resolution" && (
                        <div className="flex flex-col lg:flex-row gap-6 animate-in fade-in duration-300">
                            ...
                            <div className="lg:w-[70%] space-y-6">
                                ...
                                {ticket.relationships?.resolutions && ticket.relationships.resolutions.length > 0 && (
                                    <div className="space-y-4 mb-6">
                                        <h3 className="text-lg font-bold text-slate-800">Resoluciones Previas</h3>
                                        {ticket.relationships.resolutions.map(res => (
                                            <div key={res.id} className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm">
                                                <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-100">
                                                    ...
                                                    <div className="text-xs text-slate-500 flex items-center gap-1.5 font-medium">
                                                        ...
                                                        {new Date(res.attributes.created_at).toLocaleString()}
                                                    </div>
                                                </div>
                                                <div 
                                                    className="prose prose-sm prose-slate max-w-none text-slate-700"
                                                    dangerouslySetInnerHTML={{ __html: res.attributes.description }}
                                                />
                                            </div>
                                        ))}
                                    </div>
                                ...
                                {/* Editor Card */}
                                <form onSubmit={onResolveSubmit} className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col">
                                    {/* Header */}
                                    <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                                        <h3 className="text-lg font-bold text-slate-800">Añadir Resolución</h3>
                                        {isResolving && <Loader2 className="w-4 h-4 animate-spin text-indigo-500" />}
                                    </div>

                                    {resolveError && (
                                        <div className="mx-6 mt-4 p-4 bg-rose-50 border border-rose-200 rounded-xl flex gap-3 text-rose-600">
                                    ...
                                    <div className="p-0 bg-white">
                                        <ReactQuill 
                                            theme="snow"
                                            value={resolveForm.watch("description")}
                                            onChange={(val) => resolveForm.setValue("description", val, { shouldValidate: true })}
                                    ...
                                    <div className="px-6 pb-2 pt-14">
                                        {resolveForm.formState.errors.description && (
                                            <p className="text-xs font-medium text-rose-500 mb-2">{resolveForm.formState.errors.description.message}</p>
                                        )}
                                    </div>
                                    ...
                                    <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end">
                                        <button 
                                            ...
                                            className="px-6 py-2.5 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-70 flex items-center gap-2"
                                        >
            ...
            <CategorizationModal
                ...
                ticketId={id}
                currentCategory={ticket.attributes.ticket_category_id}
                currentPriority={ticket.attributes.ticket_priority_id}
                onSuccess={() => {
                    refetch();
                }}
            />
            ...
            <AssignTechnicianModal
                ...
                onClose={() => setIsAssignTechnicianModalOpen(false)}
                ticketId={id}
                currentTechnicians={technicians.map(t => ({ id: Number(t.relationships.user.id), role_id: Number(t.relationships.role.id) }))}
                onSuccess={() => {
                    refetch();
                }}
            />
